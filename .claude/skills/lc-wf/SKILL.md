...
所有 Lincoln SOP 工作流统一通过 `lc-wf-*` 启动，底层命令为 `python3 "${CLAUDE_PLUGIN_ROOT:-${LINCOLN_HOME:-$HOME/.lincoln/current}}/scripts/lincoln_workflow.py`。"

## 命令映射

| 触发 | 命令 | 模式 |
|------|------|------|
| `lc-wf-list` | `python3 "${CLAUDE_PLUGIN_ROOT:-${LINCOLN_HOME:-$HOME/.lincoln/current}}/scripts/lincoln_workflow.py" list` | — |
...
1. 运行 `python3 "${CLAUDE_PLUGIN_ROOT:-${LINCOLN_HOME:-$HOME/.lincoln/current}}/scripts/stage_loader.py" --stage <current_stage> --action validate-entry` 进行准入校验。
