...
def save_state(state: dict[str, Any], path: Path | None = None) -> None:
    ...
    write_package_index(state, path)
...
def action_handoff_report(stage_id: str, state: dict[str, Any], state_file: Path) -> str:
    """Generate <process_slug>/handoffs/lc-handoff-{stage}.md and refresh pm-to-ux.handoff.yaml status."""
    template_name = state.get("workflow", {}).get("template")
    workflow = load_workflow(template_name)
    ...
    lines.append("*PM→UX handoff contract status is also refreshed in `handoffs/pm-to-ux/pm-to-ux.handoff.yaml`.*")
    ...
    # Refresh the PM→UX handoff contract status fields if the contract exists.
    pm_to_ux_path = handoff_dir / "pm-to-ux" / "pm-to-ux.handoff.yaml"
    if pm_to_ux_path.exists():
        try:
            contract = load_yaml(pm_to_ux_path)
            if isinstance(contract, dict):
                approval = contract.setdefault("approval", {})
                gate_passed = bool(stage_state.get("gate_passed"))
                approval["pm_confirmed"] = gate_passed
                if gate_passed:
                    approval["approved_at"] = stage_state.get("completed_at") or now_iso()
                    approval["approved_by"] = stage_state.get("approved_by") or "human-pm"
                else:
                    approval["approved_at"] = ""
                    approval["approved_by"] = ""
                save_yaml(pm_to_ux_path, contract)
                print(f"Refreshed PM→UX handoff contract: {pm_to_ux_path.relative_to(project_root)}")
        except Exception as exc:
            print(f"WARNING: could not refresh PM→UX handoff contract: {exc}", file=sys.stderr)
